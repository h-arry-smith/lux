...
- [X] Create a dev console which allows interaction with the lighting state while it's running.
...
- [X] Create a file watcher that watches for changes in all those files
  - [X] Handle Added Files
  - [X] Handle Removed Files
  - [ ] Handle 'always eval' files - definitions folder?
...
- [X] Implement the global level time directives in main.lux
- [X] Create a cuelist folder and cues inside of it
- [X] Load a cuelist (by default for this purpose) and enter the first cue successfully
